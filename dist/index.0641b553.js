        ...
        response.data.map((country)=>{
            ...
            switch(country.region){
                ...
                case "Asie":
                    ...
                    break;
                case "Oceania":
                    countryName.style.color = "purple";
                    break;
                case "Europe":
                    countryName.style.color = "yellow";
                    break;
                default:
...
async function fetchSpecData(country) {
    try {
        const response = await (0, _axiosDefault.default).get(`https://restcountries.com/v2/name/${country}`, {
            params: {
                userInput: country
            }
        });
        ...
        response.data.map((country)=>{
            const { flags: { png  } , name , capital , subregion , population , currencies , languages  } = country;
            let coinString;
            currencies.map((coin)=>{
                if (currencies.length < 2) coinString = `and you can pay with ${coin.name}`;
                else for(let i = 0; i < currencies.length; i++)coinString = `and you can pay with ${coin.name[0]} and ${coin.name[1]}`;
            });
            let soundOfTalks;
            const soundOfTalksArray = languages.map((sound)=>{
                return sound.name;
            });
            if (languages.length < 2) soundOfTalks = `They speak ${soundOfTalksArray[0]}`;
            else if (languages.length === 2) soundOfTalks = `They speak ${soundOfTalksArray[0]} and ${soundOfTalksArray[1]} `;
            else soundOfTalks = `They speak ${soundOfTalksArray[0]} , ${soundOfTalksArray[1]} and ${soundOfTalksArray[2]} `;
            //create new elements with attributes
